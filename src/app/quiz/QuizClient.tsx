      ...
      setAnswers((prev) => {
        ...
        if (current.multi) {
          const arr = Array.from(
            (Array.isArray(next[step]) ? next[step] : []) as string[]
          );
    ...
    return (
      <div className="mx-auto max-w-[720px] px-5 py-10" dir="rtl">
        ...
        <div className="mt-8 flex justify-center gap-3">
          <button
            onClick={restart}
            className="cursor-pointer rounded-lg border border-[var(--line)] px-4 py-2 text-sm text-[var(--mut)] hover:border-[var(--gold)] hover:text-[var(--ink)]"
          >
            إعادة الاختبار
          </button>
          <Link
        ...
        <>
            ...
            {current.options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={
                    'flex flex-col items-center gap-2 rounded-xl border p-4 text-center transition-colors cursor-pointer ' +
                    (selected
          ...
          <div className="mt-8 flex justify-between">
            <button
              onClick={goPrev}
              className={
                'cursor-pointer rounded-lg border border-[var(--line)] px-4 py-2 text-sm text-[var(--mut)] hover:border-[var(--gold)] hover:text-[var(--ink)] ' +
                (step === 0 ? 'invisible' : '')
              }
            >
              السابق
            ...
            <button
              onClick={goNext}
              disabled={!canNext}
              className="cursor-pointer rounded-lg bg-[var(--gold)] px-4 py-2 text-sm font-bold text-[#171009] hover:bg-[var(--gold)]/80 disabled:opacity-40"
            >
